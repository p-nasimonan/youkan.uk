...
OUTPUT="photo_dates.csv"
echo "ファイル名,撮影日時" > "$OUTPUT"
...
for ext in jpg jpeg png heic JPG JPEG PNG HEIC; do
    for file in *.$ext(N); do
        ...
        if [[ "$raw_date" == "(null)" || -z "$raw_date" ]]; then
            ...
            creation_date=$(date -j -f "%Y-%m-%d %H:%M:%S %z" "$raw_date" "+%Y/%m/%d %H:%M:%S" 2>/dev/null)
        fi
        
        echo "\"$filename\",\"$creation_date\"" >> "$OUTPUT"
    done
done

echo "実行完了。このウィンドウは閉じてください。"
